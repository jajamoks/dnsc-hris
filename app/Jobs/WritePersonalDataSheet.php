...
class WritePersonalDataSheet extends Job implements SelfHandling
{
    ...
    protected function writePersonalDataSheet()
    {
        ...
        $pds_file = Excel::load($path, function ($reader) {
            ...
            if ($employeeEducations) {
                ...
                foreach ($this->employee->employee_educations()->graduateStudies()->get() as $college) {
                    $educations->push($college);
                }
            ...
            if ($this->employee->trainings) {
                $row       = 16;
                $trainings = $this->employee->trainings()->finishedTrainings()->includedInPds()->get()->sortByDesc('start')->take(15);
